...
function createTextWithBG(text,posX,posY,fontSize,textColor,fontCode){
	...
	if(fontCode){
		...
		rectStartPosX=posX-tWidth/2-fontSize/5-fontSize/6+5;
		rectStartPosY=posY-fontSize-fontSize/4.5-5;
		rectWidth=tWidth+fontSize/2+5;
		...
		// tWidth=0.733*tWidth;
		rectStartPosX=posX-tWidth/2-fontSize/5-fontSize/6+5;
		rectStartPosY=posY-fontSize-fontSize/4-fontSize/6-5;
		rectWidth=tWidth+fontSize/2+5;
		rectHeight=fontSize+fontSize/2;
	}
	// console.log(tWidth);
	...
	ctx.fillRect(rectStartPosX,rectStartPosY,rectWidth,rectHeight);
	ctx.fillStyle="rgb(255,255,255)";
	ctx.fillRect(rectStartPosX+5,rectStartPosY+5,rectWidth-10,rectHeight-10)
	ctx.fillStyle="rgb(232,97,1)";
	ctx.fillRect(rectStartPosX+10,rectStartPosY+10,rectWidth-20,rectHeight-20)
	ctx.fillStyle=textColor;
	ctx.fillText(text,posX-tWidth/2,posY-fontSize/2.4);
	// ctx.restore();
	return [rectStartPosX,rectStartPosY,rectWidth,rectHeight];

}

function createTextWithoutBG(text,posX,posY,fontSize,textColor,fontCode){
	// ctx.save();
	// FONT CODES: 0-> sans-serif 1-> Flappy-Bird-Regular
	var tWidth,rectStartPosX,rectStartPosY,rectWidth,rectHeight;
	if(fontCode){
	...
	ctx.fillText(text,posX-tWidth/2+2,posY-fontSize/2.4+2);
	ctx.fillStyle=textColor;
	ctx.fillText(text,posX-tWidth/2,posY-fontSize/2.4);
	// ctx.restore();
	// return [rectStartPosX,rectStartPosY,rectWidth,rectHeight];
